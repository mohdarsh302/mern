    ...
    <div className="col-lg-3 d-none d-lg-block">
      ...
      <nav className="collapse position-absolute navbar navbar-vertical navbar-light align-items-start p-0 bg-light" id="navbar-vertical" style={{width: 'calc(100% - 30px)', zIndex: 999}}>
        <div className="navbar-nav w-100">
          ...
          <Link href className="nav-item nav-link" to={"/categories"}>All Categories</Link>
          {categories?.map((c) => (
            <Link href className="nav-item nav-link"  to={`/category/${c.slug}`}> {c.name}</Link>
           ))}
    ...
    <div className="col-lg-9">
      <nav className="navbar navbar-expand-lg bg-dark navbar-dark py-3 py-lg-0 px-0">
        ...
        <div className="collapse navbar-collapse justify-content-between" id="navbarCollapse">
          <div className="navbar-nav mr-auto py-0">
            ...
            <div className="nav-item dropdown">
              ...
              <div className="dropdown-menu bg-primary rounded-0 border-0 m-0">
                <Link to="/new-cart" className="dropdown-item">Shopping Cart</Link>
            ...
            <Link to="/register" className="nav-item nav-link">Register</Link>
              ...
              <>
               <div className="nav-item dropdown">
                <Link  className="nav-link dropdown-toggle" data-toggle="dropdown">{auth?.user?.name} <i className="fa fa-angle-down mt-1" /></Link>
                <div className="dropdown-menu bg-primary rounded-0 border-0 m-0">
                  <Link  to={`/dashboard/${
                            auth?.user?.role === 1 ? "admin" : "user"
                          }`}className="dropdown-item">Dashboard</Link>
                  <Link  onClick={handleLogout}
                          to="/login" className="dropdown-item">Logout</Link>
                </div>
              </div>
              </>
              )}
          ...
          {/* {!auth?.user ? (
            <>
             <NavLink to="/login" className="btn px-2 text-secondary">Login</NavLink>
             <NavLink to="/register" className="btn px-2 text-secondary">Register</NavLink>
            </>
             ) : (
              <>
               <div className="nav-item dropdown">
          ...
          >
            {auth?.user?.name} <i className="fa fa-angle-down mt-1" />
          </Link>
          <div className="dropdown-menu bg-primary rounded-0 border-0 m-0">
            <Link
            ...
            {/* <NavLink  to="/cart" className="btn px-0 ml-3">
              ...
              <span className="badge text-secondary border border-secondary rounded-circle" style={{paddingBottom: 2}}>{cart?.length} showZero offset={[10, -5]}</span>
            </NavLink> */}
            <NavLink  to="/new-cart" className="btn px-0 ml-3">
              <i className="fas fa-shopping-cart text-primary" />
